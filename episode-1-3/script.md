...
小伙伴们大家好，上期视频，我们已经完成了正则表达式的解析。本期视频我们将一起学习有穷状态机的结构和原理以及状态跳转图的生成算法。

# 1. 有穷状态机

首先是有穷状态机。简单来讲，有穷状态机，或者说确定有穷状态机，是图灵机的一种简化模型。

它记录一个当前状态，工作在一个输入串上。有穷状态机在逐个读取输入单元的过程中在有限的若干个状态之间跳转。

在词法分析的任务中，这有限的若干个状态被描述在一个以状态为节点，以可行跳转为边的状态跳转图上。

若在某些状态下，从初始状态到当前状态的路径已经描述了一个词法规则可以接受的单词，这个状态上还会额外记录这个可以接受的单词的ID，这样的状态被称为接受状态。

状态跳转图与词法规则对应，一份词法规则可以描述若干单词的书写模式，一份状态跳转图上也会存在多个接受状态，这些接受状态可能记录着不同的单词ID。

词法分析器每次从初始状态开始工作，从输入缓冲逐个读取字符并跳转状态。每次成功分析一个单词或报告一个词法错误后，都会回到初始状态等待下一次触发。

在每个状态上，针对不同的输入，状态机可以查询状态跳转图来确定要执行的动作。

- 若存在与当前输入对应的跳转边，则状态机将当前输入存入单词缓冲，然后按照状态跳转图的指示跳转到下一个状态继续分析后续输入。
- 若当前输入没有对应的输入边，但当前状态是一个接受状态。则状态机将已存入缓冲的字符串打包成一个单词完成本次分析，单词ID就是当前接受状态上记录的单词ID。
- 若当前输入没有对应的输入边，且当前状态也不是接受状态，则词法分析器遭遇了一个词法错误。我们将当前见过的字符串连同当前这个引起错误的输入字符整理起来报告一个词法错误。

了解了有穷状态机的工作原理后，我们就可以开始动手实现一个有穷状态机了。
